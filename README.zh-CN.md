...
右上角可切换中文 / English，语言偏好保存在当前浏览器中。界面语言不修改提示词、模型或历史内容。
...
自动测试无需浏览器、账号或网络，覆盖策略判定、配置校验与冲突处理、隔离安装、重复升级、卸载保留数据，以及翻译覆盖。测试使用临时目录，不修改你的实际安装。

可选界面验收：调试端口可用时执行 `node tests/i18n-browser.mjs`，使用模拟数据，不向 ChatGPT 发送消息。
